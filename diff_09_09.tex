...
%DIF DEL rnni_geometry_v_1_1.tex   Thu Sep  9 11:23:03 2021
%DIF ADD rnni_geometry.tex         Thu Sep  9 11:59:50 2021
...
Computing \DIFdelbegin \DIFdel{distances in }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend $\nni$ \DIFaddbegin \DIFadd{distance between two trees }\DIFaddend is $\np$-hard \autocite{Dasgupta2000-xa}, and the proof relies on the fact that this tree space does not have the cluster property \autocite{Li1996-zw}.
...
\begin{proof}
	...
	We furthermore assume that there is no pair of trees $T',R'$ with \DIFdelbegin \DIFdel{$d(T',R') < d(T,R)$ }\DIFdelend \DIFaddbegin \DIFadd{$d_{\rnni}(T',R') < d_{\rnni}(T,R)$ }\DIFaddend that shares a cluster $C'$ and is connected by a shortest path $p'$ that does not preserve $C'$.
	...
	We now compare distances \DIFdelbegin \DIFdel{$d(T,R)$ and $d(T',R)$ }\DIFdelend \DIFaddbegin \DIFadd{$d_{\rnni}(T,R)$ and $d_{\rnni}(T',R)$ }\DIFaddend by using properties of $\findpath$.
	...
	\DIFdelbegin \DIFdel{First we compare $\fp(R,T)$ and $\fp(R,T')$.
	All trees on these two paths }\DIFdelend \DIFaddbegin \DIFadd{Therefore, we first show that all nodes with rank less than $i$ induce the same clusters in $T$ and $R$.
	If this was not the case, then all trees on $\fp(R,T)$ and $\fp(R,T')$ }\DIFaddend coincide up to iteration $i = \rank((C)_T)$, in which the cluster considered on $\fp(R,T)$ is $C$.
	Let $R'$ denote the tree at this point of the path, meaning that $\fp(R,T)$ and $\fp(R,T')$ coincide up to this tree $R'$.
	Since \DIFdelbegin \DIFdel{$R'$ is on the shortest paths $\fp(R,T)$ and $\fp(R,T')$, the length of these paths is the sum of the length of the path up to $R'$ plus the length of the remaining part of the path, respectively.
	It follows $d(T,R) = d(R,T) = |\fp(R,T)| = |\fp(R,R')| + |\fp(R',T)| = d(R,R') + d(R', T)$ and $d(T',R) = d(R,T') = |\fp(R,T')| = |\fp(R,R')| + |\fp(R',T')| =  d(R,R') + d(R', T')$.
	}%DIFDELCMD < 

%DIFDELCMD < 	%%%
\DIFdel{Now consider $\fp(T', R')$ to evaluate $d(R', T')$.
	As }\DIFdelend $\findpath$ \DIFdelbegin \DIFdel{preserves clusters (}\DIFdelend \DIFaddbegin \DIFadd{preserves clusters (}\DIFaddend \autoref{lemma:fp_cluster}), \DIFaddbegin \DIFadd{$R'$ has the cluster }\DIFaddend $C$\DIFdelbegin \DIFdel{is present in every tree on }\DIFdelend \DIFaddbegin \DIFadd{.
	...
	\DIFadd{We now show that by the minimality assumption on $T$ and $R$, $C$ is induced by the node of rank $i$ in $R$.
	We therefore assume to the contrary that $C$ is induced by a node of rank greater than $i$ in $R$.
	Then the first cluster considered on }\DIFaddend $\fp(T,R)$ \DIFdelbegin \DIFdel{up to and including $R'$.
	}\DIFdelend \DIFaddbegin \DIFadd{and $\fp(T',R)$ is a cluster $D$ induced by $(R)_i$ that does not intersect $C$.
	...
	Therefore, the most recent common ancestor of $D$ has rank greater than $i+1$ in both $T$ and $T'$, as $D$ would intersect $C$ if $(D)_T$ or $(D)_{T'}$ had rank $i$ or $i+1$ (see }\autoref{fig:nni_neighbours_clusters}\DIFadd{).
	...
	\DIFaddend The first iteration of $\findpath$ applied to the pair of trees \DIFdelbegin \DIFdel{$(T',R')$ }\DIFdelend \DIFaddbegin \DIFadd{$(T',R)$ }\DIFaddend hence considers the cluster $C$\DIFdelbegin \DIFdel{, as all cluster induced by nodes below $(C)_{T'}$ coincide in $R'$ and $T'$}\DIFdelend .
	...
	Therefore, $T$ is the first tree following $T'$ on $\fp(T',R')$, resulting in \DIFdelbegin \DIFdel{$|\fp(T',R')| = |\fp(T',T)| + |\fp(T,R')|$ and hence $d(T',R') = 1 + d(T,R')$.
	With the observations $d(T,R) = d(R,R') + d(R',T)$ and $d(T',R) = d(R,R') + d(R',T')$ it follows $d(T',R) = d(R,R') + d(R',T') = d(R,R') + d(T,R') + 1 = d(T,R) + 1$}\DIFdelend \DIFaddbegin \DIFadd{$|\fp(T',R)| = |\fp(T',T)| + |\fp(T,R)|$ and hence $d_{\rnni}(T',R) = 1 + d_{\rnni}(T,R)$}\DIFaddend .
	From the assumption that $T'$ is the first tree on a shortest path from $T$ to $R$ we can however \DIFdelbegin \DIFdel{follow $d(T',R) = d(T,R) - 1$}\DIFdelend \DIFaddbegin \DIFadd{infer $d_{\rnni}(T',R) = d_{\rnni}(T,R) - 1$}\DIFaddend , which leads to a contradiction.
...
\begin{proof}
	...
	We prove the theorem by showing that there is a caterpillar tree $T'$ that is a neighbour of $T$ and closer to $R$ than $T$\DIFaddbegin \DIFadd{, that is, $d_{\dct_m}(T', R) = d_{\dct_m}(T, R) - 1$}\DIFaddend .
	The existence of a shortest path consisting only of caterpillar trees between $T$ and $R$ follows \DIFaddbegin \DIFadd{from this property }\DIFaddend inductively.
	...
	Let furthermore $a_j \in \{a_1, \ldots, a_{m+2}\}$ be the leaf directly \DIFdelbegin \DIFdel{'}\DIFdelend \DIFaddbegin \DIFadd{`}\DIFaddend above' $a_k$ in $T_r$, i.e.\DIFaddbegin \DIFadd{\ }\DIFaddend $\rank(a_j)_{T_r} = \rank(a_k)_{T_r} + 1$.
	Note that with the definition of $a_k$ it immediately follows that $a_j$ is \DIFdelbegin \DIFdel{'}\DIFdelend \DIFaddbegin \DIFadd{`}\DIFaddend below' $a_k$ in $R_r$ ($\rank(a_j)_{R_r} < \rank(a_k)_{R_r}$).
...
\begin{theorem}
	...
	Let $T$ and $R$ be caterpillar trees in $\rnni$ such that \[1 = \rank(a_1)_R = \rank(a_2)_R < \rank(a_3)_R < \ldots < \rank(a_n)_R = n-1\DIFaddbegin \DIFadd{.}\DIFaddend \]
	\DIFdelbegin \DIFdel{and
	}\DIFdelend \DIFaddbegin \DIFadd{Define
	}\DIFaddend \[P(T,R) = \{(a_i,a_j)\ |\  \rank(a_i)_T < \rank(a_j)_T \text{ and } \rank(a_i)_R > \rank(a_j)_R\},\]
	\begin{align*}
		M(T,R) = &\{a_i\ |\  \textnormal{for all } l \textnormal{ with } \rank(a_l)_T \leq \rank(a_i)_T \textnormal{ it is } \rank(a_l)_R > \rank(a_i)_R\} \\
		& \cap \{a_i \ |\  \rank(a_i)_T < \min\{\rank(a_1)_T, \rank(a_2)_T\}\}\DIFaddbegin \DIFadd{.
	}\DIFaddend \end{align*}
	Then for ${m(T,R) = |M(T,R)|}$ and ${p(T,R) = |P(T,R)|}$:
	\[d\DIFaddbegin \DIFadd{_{\rnni}}\DIFaddend (T,R) = p(T,R) - m(T,R).\]
\end{theorem}
...
\DIFaddbegin \DIFadd{The caterpillar trees $T$ and $R$ in }\autoref{fig:tsp_caterpillar} \DIFadd{for example have $P(T,R) = \{(a_1,a_3),(a_1,a_4),(a_1,a_5),(a_2,a_3),(a_2,a_4),(a_2,a_5),(a_3,a_4),(a_3,a_5)\}$ and $M(T,R) = \{a_3,a_4\}$.
}\DIFaddend 

\begin{proof}
	Let $T$ and $R$ be caterpillar trees in $\rnni$ as described above and let $\widehat d(T,R) := p(T,R) - m(T,R)$.
	For proving \DIFdelbegin \DIFdel{$\widehat d(T,R) = d(T,R)$ }\DIFdelend \DIFaddbegin \DIFadd{$\widehat d(T,R) = d_{\rnni}(T,R)$ }\DIFaddend it is sufficient to show that there is a caterpillar tree $T'$ that is neighbour of $T$ such that $\widehat d(T',R) = \widehat d(T,R) - 1$.
	Then it follows inductively that \DIFdelbegin \DIFdel{$\widehat d(T,R) = d(T,R)$}\DIFdelend \DIFaddbegin \DIFadd{$\widehat d(T,R) = d_{\rnni}(T,R)$}\DIFaddend , because the sets $P(T,T)$ and $M(T,T)$ are empty.
	...
	In the proof of \autoref{thm:caterpillar_convex_dtt} we saw that it follows \DIFdelbegin \DIFdel{$d(T',R) = d(T,R) - 1$}\DIFdelend \DIFaddbegin \DIFadd{$d_{\rnni}(T',R) = d_{\rnni}(T,R) - 1$}\DIFaddend .	
...
%DIFDELCMD < %%%
\DIFdel{The }\emph{\DIFdel{partition lattice}} %DIFAUXCMD
...
A }\emph{\DIFdel{chain}} %DIFAUXCMD
\DIFdel{in a lattice is a list $[X_0, \ldots, X_k]$ with $X_0 \leq X_1 \leq \ldots \leq X_k$.
The }\emph{\DIFdel{length}} %DIFAUXCMD
\DIFdel{of the chain $[X_0, \ldots, X_k]$ is $k$, the number of its elements minus one, and such a chain is called }\emph{\DIFdel{maximum chain}} %DIFAUXCMD
...
\DIFdel{The $\rnni$ graph on $n$ leaves is isomorphic to the graph $G_{\Pi}$ of maximum chains of the partition lattice $\Pi_n$ where two maximal chains are connected by an edge if and only if they differ by exactly one partition.
	...
	}%DIFDELCMD < \label{thm:partition_lattice}
...
\DIFdel{There is a one-to-one relation between ranked trees and maximum chains in a partition lattice.
	...
	In }%DIFDELCMD < \autoref{fig:partition_lattice4} %%%
...
%DIFDELCMD < 	%%%
\DIFdel{An $\nni$ move on a tree $T$ that results in a tree $T'$ also only changes one cluster in the cluster representation of a tree $T$, as we already described in }%DIFDELCMD < \autoref{fig:nni_neighbours_clusters}%%%
\DIFdel{.
...
\DIFdel{Let us now consider the second case, where $X_i$ results from $X_{i-1}$ by uniting two sets $C_j$ and $C_k$, and $X_{i+1}$ results from $X_i$ by uniting the sets $C_j \cup C_k$ and $C_l$.
	...
	With }%DIFDELCMD < \autoref{fig:nni_neighbours_clusters} %%%
...
%DIFDELCMD < %%%
\DIFdel{With the bijection between maximum chains in the partition lattice and ranked trees, as described in the proof of }%DIFDELCMD < \autoref{thm:partition_lattice}%%%
...
%DIFDELCMD < \centering
%DIFDELCMD < \includegraphics[width=0.9\textwidth]{partition_lattice4}
...
%DIFAUXCMD
%DIFDELCMD < \label{fig:partition_lattice4}
