...
\todo{cite Gavryushkin, Whidden, Matsen paper on discrete time-trees}
...
The number of discrete time-trees with root time less or equal to $m$ is $\frac{(n-1)!n!m^{n-1}}{2^{n-1}}$.
Discrete time-trees with root time $n-1$ are called \emph{ranked trees}.

\summary{Defining the tree space $\dtt$}

As a generalisation to the graph $\rnni$ of ranked trees (\autocite{Collienne2020-iu}), we define the graph $\dtt_m$ of discrete time-trees as follows.
The nodes of $\dtt_m$ are discrete time-trees with root time less pr equal to $m$.
A tree $R$ is connected to a tree $T$ by an edge, if performing one of the following operations on $T$ results in $R$:
\begin{enumerate}
	\item An \emph{$\nni$ move} connects trees $T$ and $R$ if there is an edge $e$ in $T$ and an edge $f$ in $R$, both of length one, such that shrinking $e$ and $f$ to nodes results in isomorphic trees. \todo{define isomorphic!}
...
\end{enumerate}
\todo{This could be explained a bit better. Maybe we want to speek about intervals?}
Note that a length move in $T$ on a node $v$ can only happen if the time of $v$ after the length move is not already assigned to an internal node in $T$.
It is also important to see that our definition of $\dtt_m$ differs from the definition in \tocite.
As the $\rnni$ graph is the graph $\dtt_m$ for $m= n-1$, length moves are not possible in this graph.

Things we need to define:
\begin{itemize}
	\item intervals, to make explanation of length move and how they are replaced in order to use FP easier?
	\item $(S)_T$, also for one-element sets $S$
	...
	\item $d(T,R)$ is the length of a shortest path, index $\dtt$ or $\rnni$ for the space we are currently in
...
\summary{How distances in $\dtt$ and $\rnni$ are related}

\begin{lemma}
	\todo{How do we want to phrase this? Underlying or corresponding? -- or even only use the index  $d_{\rnni}$ to emphasise that we consider the corresponding trees in $\rnni$}
	Let $T$ and $R$ be discrete time-trees and $T'$ and $R'$ the underlying ranked trees.
	Then the distance between $T$ and $R$ in $\dtt$ is:
	\[d_{\dtt}(T,R) = d_{\rnni}(T,R) + l_{T,R},\]
	\todo{Maybe we introduce these moves so that we can still call them length moves, even if on $\findpath$ they actually are rank moves?}
	where $l_{T,R}$ is the number of rank moves on $\fp(T,R)$ that involve a node of the additional caterpillar tree.
\end{lemma}
