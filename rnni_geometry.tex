...
\begin{corollary}
	The distance between two caterpillar trees can be computed in $\O(m + n \log n)$ in $\rnni$.
...
\begin{proof}
	...
	Since the sum that gives the number of length moves can be computed in time linear in $m$ and the $\rnni$ distance between $\hat T$ and $\hat R$ in $\O(n \log n)$, the distance between $T$ and $R$ in $\dtt_m$ can be computed in $\O(m + n \log n)$.
\end{proof}
