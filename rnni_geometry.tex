...
\todefine{
	\begin{itemize}
		\item We need to mention that $m$ is ficed when we compute distances between two trees
		...
		\item rank interval for Lemma~\ref{lemma:nni_path_to_caterpillar} -- if needed, in corresponding section
...
\todo{Do we want to use the notion 'tree space' -- define it!}
...
\begin{proof}
	\todo{Do we need this assumption on $R$ anywhere?}
	...
	\todo{don't use $a_k$ and $a_{k+1}$, they don't need to be neighbours in $R$}
	Let $a_k, a_{k+1}$ be the labels of leaves that exchange their position between $T$ and $T'$, such that $\rank(a_k)_T < \rank(a_{k+1})_T$.
	...
	It remains to show $(M_{T'} \setminus M_T) \neq \{a_k, a_{k+1}\}$, from which we can follow $m_{T'} \leq m_T - 1$.

	To prove this, by showing that if $a_k \notin M_T$, it follows $a_k \notin M_{T'}$.
	...
	\begin{align}
		\forall j \text{ with } \rank(a_j)_T \leq \rank(a_i)_T: \rank(a_j)_R > \rank(a_i)_R \label{condition1}\\
	...
	It follows that there is a $j$ such that $\rank(a_j)_T < rank(a_k)_T$ and $\rank(a_k)_R > \rank(a_j)_R$.
	It immediately follows that the same condition is violated for $a_k$ in $T'$, because the $\nni$ move exchanging $a_k$ and $a_{k+1}$ preserves the relationship of $a_k$ and $a_j$.
	Therefore, it is $a_k \notin M_{T'}$.

	Let us now assume that Condition~\ref{condition2} is violated for $a_k$ to not be in $M_T$.
	It follows $\rank(a_k)_T \geq \min\{\rank(1)_T, \rank(2)_T\}$.
	As only $a_k$ and $a_{k+1}$ exchange between $T$ and $T'$, the same inequality holds for $a_k$ in $T'$, resulting in a violation of Condition~\ref{condition2} for $a_k$ in $T'$ and hence a contradiction to $(M_{T'} \setminus M_T) = \{a_k, a_{k+1}\}$.
	We can conclude that if $a_k \notin M_T$, it follows $a_k \notin M_{T'}$.
	Therefore, it is $a_k \notin (M_{T'} \setminus M_T)$, showing that $(M_{T'} \setminus M_T) \neq \{a_k, a_{k+1}\}$, and hence $m_{T'} \leq m_T + 1$.

	It remains to show that if $p_{T'} = p_T - 1$, it cannot be $m_{T'} = m_T + 1$, in order to prove Inequality~\ref{eq:distance_proof}.
	We assume $p_{T'} = p_T - 1$, hence $(a_k, a_{k+1})$ is a transposition in $T$, meaning that $\rank(a_k)_R > \rank(a_{k+1})_R$.
	As $a_k$ and $a_{k+1}$ are the only leaves whose membership can change between $M_T$ and $M_{T'}$, it suffices to show that none of them can be in $M_{T'} \setminus M_T$.

	We already saw above that it cannot be $a_k \in (M_{T'} \setminus M_{T})$.
	We hence assume $a_{k+1} \in (M_{T'} \setminus M_T)$.
	Since $a_{k+1} \notin M_T$ it follows that one of the two conditions is not fulfilled for $a_{k+1}$ in $T$.
	If this is the first one, it also is violated in $M_T$, as $a_k$ cannot be the reason it is violated in $T$ since $\rank(a_k)_R > \rank(a_{k+1})_R$.
	If Condition~\ref{condition2} is violated for $a_{k+1}$ in $T$, but not in $M_{T'}$ (as $a_{k+1} \in M_{T'}$), it follows again that $a_k$ is not the reason this inequality is violated as $\rank(a_k)_{T'} > \rank(a_{k+1})_{T'}$.
	It hence follows that Condition~\ref{condition2} is also violated for $a_{k+1}$ in $T$, and therefore, $a_{k+1} \notin (M_{T'} \setminus M_T)$.

	Concluding, it is $a_k, a_{k+1} \notin (M_{T'} \setminus M_T)$, and we can follow that if $p_{T'} = p_T - 1$, it is $m_{T'} < m_T + 1$, which concludes this proof.
\end{proof}
...
\todo{This is mainly relevant for $RNNI(\infty)$. As it is a statement about $\rnni$, we could still keep it here, if needed}
\begin{lemma}
...
\todo{What about the diameter of $\dtt$?}
