...
\begin{theorem}
	...
	\[{M_T} := \{i| (\forall k \text{ with } \rank(k)_T \leq \rank(i)_T: i < k) \text{ and } \rank(i)_T \leq \min\{\rank(1)_T, \rank(2)_T\}\}\]
...
\begin{proof}
	...
	To proof the correctness of \autoref{eq:distance_proof}, we assume to the contrary that there are is a neighbour $T'$ of $T$ such that $\hat d(T',R) < \hat d(T,R) - 1$.
	As an $\nni$ move between caterpillar trees $T$ and $T'$ only exchanges two leaves, at most one transposition of $T$ can be resolved in $T'$, resulting in $p_{T'} \geq p_T - 1$.
	Furthermore, it is $m_{T'} \leq m_T + 1$, as we explain in the following.
	Since the only change between $T$ and $T'$ is that two leaves $a_k, a_{k+1}$ exchange their position, these are the only elements that could be added to $M_T$ to receive $M_{T'}$.
	This however is not possible.
	\todo{Define the notion $(a_k)_T$ and $\rank$}
	If neither of these leaves is in $M_T$, then there exists a leaf $a_j$ with $\rank(a_j)_T < \rank(a_k)_T < \rank(a_{k+1})_T$ such that $a_j > \min{a_k,a_{k+1}}$.
	Since the position of $a_j$ does not change between $T$ and $T'$, it follows that $a_k$ and $a_{k+1}$ can not be members of $M_{T'}$.
	Hence it is $m_{T'} \leq m_T + 1$.

	As $p_{T'} \geq p_T - 1$ and $m_{T'} \leq m_T + 1$, the only case where Inequality~\ref{eq:distance_proof} can be violated is if $p_{T'} = p_T - 1$ and $m_{T'} = m_T + 1$.
	This means that $a_k, a_{k+1}$ is a transposition in $T$, meaning that $a_k > a{k+1}$.
	We now consider how resolving this transposition changes the set $M_T$.
	If $a_k \notin M_T$, then it will not be in $M_T$ after the move either.
	If in this case also $a_{k+1} \notin M_T$, then it also is not in $M_{T'}$.
	Therefore, $m_{T'}$ cannot be greater than $m_T$ if $a_k \notin M_T$.
	If on the other side $a_k \in M_T$, it follows that $a_k \notin M_{T'}$, as $a_{k+1} > a_k$ and $\rank(a_{k+1})_{T'} < \rank(a_k)_{T'}$, which again results in $m_{T'}$ being less or equal to $m_T$.

	We can conclude that if $p_{T'} \geq p_T - 1$, it is $m_{T'} < m_T + 1$, and with $p_{T'} \geq p_T - 1$ and $m_{T'} \leq m_T + 1$ it follows
	\[\hat d(T',R) = p_{T'} - m_{T'} \geq p_T - 1 - m_T = \hat d(T,R) - 1\].
	\todo{What happens if the transposition happens 'above' $1$ or $2$?}
