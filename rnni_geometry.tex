...
\begin{proof}
	...
	The trees in \todo{figure} provide an example where the path computed by $\findpath$ actually has length $m(n-1) - \frac{n(n-1)}{2}$, proving that this upper bound for distances is tight and hence the diameter of $\dtt_m$.
\end{proof}
...
\todo{This needs to be expanded. Can we come up with a more intuitive description of what the radius of a graph is?}
