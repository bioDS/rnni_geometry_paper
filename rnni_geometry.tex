...
\begin{proof}
	...
	\todo{the forumla for the number of length moves is not correct (counterexample in my lab seminar talk nov2020)}
...
\begin{proof}
	With \autoref{lemma:dtt_path_sum} we know that the distance between two trees $T$ and $R$ in $\dtt_m$ is the sum of the minimum number $r(T,R)$ of $\rnni$ moves and the minimum number $l(T,R)$ of length moves on any shortest path between them.
	...
	We have already seen in the proof of \autoref{lemma:dtt_path_sum} that it is $l(T,R) = \sum_{i = n+1}^{m+2} |\rank(a_{i})_{T_c} - \rank(a_{i})_{R_c}|$, where $T_c$ and $R_c$ are the caterpillar trees added to $T$ and $R$ when applying \autoref{alg:ranked_tree}, respectively.
