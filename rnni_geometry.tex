...
In this section we consider a property of tree spaces that is motivated by its biological meaning and seems to be connected to the complexity of computing distances in these spaces.
...
\todo{Do we want the biological motivation here as well or shall only have this in the introduction?}
This is desirable biological property as for example for a given sample of trees supporting the same evolutionary hypothesis in form of a common subtree, it is expected that the same hypothesis is supported by a summary or mean tree of the sample.
...
The following lemma gives further insights into the connection of shortest paths in $\rnni$ and $\dtt_m$ space.
It will be important for generalising geometrical properties of $\rnni$ to $\dtt_m$.

\begin{lemma}
	Any distance between two trees $T$ and $R$ in $\dtt_m$ is the sum of the minimum number of $\rnni$ moves $r(T,R)$ and the minimum number of length moves $l(T,R)$ between these trees.
...
\begin{proof}
	...
	As $\fp(T,R)$ computes a shortest path (\autoref{thm:dtt_findpath}), this follows for all shortest paths.
	...
	Therefore, the minimum number of moves needed to transform $T_c$ to $R_c$ is $\sum_{i = n+1}^{m+2} |\rank(a_{i})_{T_c} - \rank(a_{i})_{R_c}|$, which is the number rank of moves that $\findpath$ takes for these subtrees.

	To prove that the number of $\rnni$ moves $r(T,R)$ on $\fp(T,R)$ is minimal as well, we assume to the contrary that that there was a path $p$ with less than $r(T,R)$ $\rnni$ moves between $T$ and $R$.
	...
	It follows that $\findpath$ computes a shortest paths minimising both the number $r(T,R)$ of $\rnni$ moves and the number of length moves $l(T,R)$ and hence every shortest path in $\dtt_m$ has this property.
\end{proof}
...
In \autoref{thm:caterpillar_convex_rnni} and \autoref{cor:caterpillar_convex_dtt} we will see that for any two caterpillar trees there always exists a shortest paths between them that is a caterpillar path in both $\rnni$ and $\dtt_m$.
...
This is another property besides the cluster property of $\rnni$ that the $\nni$ space of unranked trees does not have \autocite{Gavryushkin2018-ol}.
Based on the convexity of the set of caterpillar trees in $\rnni$ we introduce a way to compute distances between caterpillar trees in this space in time $\O(n\log n)$ \toref, and hence better worst-time complexity than $\findpath$.
...
\begin{corollary}
	...
	\label{cor:distance_caterpillars_rnni_nlogn}
\end{corollary}
...
\begin{proof}
	...
	Since the sum that gives the number of length moves can be computed in time linear in $m$ and the $\rnni$ distance between $\hat T$ and $\hat R$ in $\O(n \log n)$ (\autoref{cor:distance_caterpillars_rnni_nlogn}), the distance between $T$ and $R$ in $\dtt_m$ can be computed in $\O(m + n \log n)$.
\end{proof}
