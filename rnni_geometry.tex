...
\begin{proof}
	...
	We hence need to prove $\hat d(T,R) = d(T,R)$.
	It is sufficient to show that for any caterpillar tree $T'$ that is neighbour of $T$ it is
	\begin{align}
	...
	For proving Inequality~\ref{eq:distance_proof} we first establish $p_{T'} \geq p_T - 1$ and then $m_{T'} \leq m_t + 1$.
	...
	Throughout this proof we call pairs $(i,j) \in P_T$ transpositions, as the order of leaves in the caterpillar trees $T$ and $T'
	$ can be seen as a permutation of their order in $R$.

	The only move possible between caterpillar trees $T$ and $T'$ is an $\nni$ move exchanging two leaves, and hence at most one transposition of $T$ can be resolved in $T'$, resulting in $p_{T'} \geq p_T - 1$.
	...
	Let us now assume that (\ref{condition2}) is violated for $a_k$ to not be in $M_T$.
	It follows $\rank(a_k)_T \geq \min\{\rank(a_1)_T, \rank(a_2)_T\}$.
	As only $a_k$ and $a_j$ exchange between $T$ and $T'$, it follows $a_j \in \{a_1, a_2\}$, resulting in a violation of (\ref{condition2}) for $a_j$ in $T'$ and hence $a_j \notin M_{T'}$.
