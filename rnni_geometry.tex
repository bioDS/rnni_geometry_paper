...
The nodes of $\dtt_m$ are trees with root time less or equal to $m$.
...
\todefine{
	\begin{itemize}
		...
		\item cherry (for lemma~\ref{lemma:max_dist_tree_rnni}) -- in caterpillar section (if needed)
...
\begin{proof}
	From Lemma~\ref{lemma:max_dist_tree_rnni} we know that there is a tree with distance $\frac{(n-1)(n-2)}{2}$ from any given tree.
	Hence the radius equals the diameter of $\rnni$.
\end{proof}

\todo{The radius of $\dtt$ is not equal to its diameter -- tree on 3 leaves with internal nodes ranked $2$ and $4$ gives counterexampleb}
