...
\begin{lemma}
	Let $T$ and $R$ be discrete time-trees and $T'$ and $R'$ the ranked trees resulting from these by ignoring times and only keeping ranks.
	The sequence of $\rnni$ moves on $\fp(T,R)$ (in $\dtt_m$) results in the path $\fp(T',R')$ (in $\rnni$).
	\label{lemma:dtt_fp_path_rnni}
\end{lemma}
...
With \autoref{lemma:dtt_fp_path_rnni} the cluster property in $\dtt_m$ follows from \autoref{thm:cluster_property_rnni}.
...
The convexity of the set of caterpillar trees in $\dtt_m$ follows from \autoref{thm:caterpillar_convex_rnni} with \autoref{lemma:dtt_fp_path_rnni}.
...
\begin{proof}
	\todo{Needs to be re-written with new Lemma 1}
	With \autoref{lemma:dtt_fp_path_rnni} we know that the distance between two caterpillar trees $T$ and $R$ in $\dtt_m$ is the sum of minimal $\rnni$ moves and the minimal number of length moves between $T$ and $R$.
	As we already saw in the proof of \autoref{lemma:dtt_fp_path_rnni}, the minimum number of length moves is $\sum_{i = n+1}^{m+2} |\rank(a_{i})_{T_c} - \rank(a_{i})_{R_c}|$, and can be computed in time linear in $m$.
