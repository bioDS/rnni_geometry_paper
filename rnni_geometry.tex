...
\begin{abstract}
Computational inference of dated evolutionary histories relies upon models of RNA, DNA, and protein sequence mutation rates.
\todo[inline]{. DB: This is not quite right. Coalescent theory states assumptions about the genealogies. It does not make assumptions about variations in evolutionary rates}
...
It has recently been shown that molecular clock, and hence coalescent, trees possess a unique geometry, different from that of classical phylogenetic tree spaces.
...
\todo[inline]{DB: Even when the molecular clock is rejected, the UNDERLYING phylogeny, with branch lengths given by durations, has to be clocklike (assuming all taxa sampled at the same time). This separation is seen clearly with coalescent theory. The theory models the genealogy for a single gene sampled from multiple genomes. The genealogy is clock-like when branch lengths are proportional to time or generation or count, but may appear non-clocklike if branch lengths are represented proportional to mutations. }
