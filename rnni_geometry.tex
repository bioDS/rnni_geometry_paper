...
Leaves of these trees may for example represent different species, or different genes within a species.
...
A popular model for reconstructing genealogical trees to infer relationships of a sample of genes or chromosomes is the coalescent model.
...
These reconstruction algorithms rely on tree search algorithm.
An objective function, such as the posterior probability or the likelihood function \tocite, are optimised by searching a tree space.
Under the coalescent model, tree spaces where internal nodes are ordered according to times, are used for tree search.
...
\begin{proof}
	...
	\todo{Tree labels in figure}
	\begin{figure}[ht]
		\includegraphics[width=0.65\textwidth]{cat_max_dist_dtt.eps}
		\caption{Trees with distance $\frac{(n-1)(n-2)}{2} + (m-n+1)(n-1)$}
