...
In \autoref{thm:caterpillar_convex_rnni} and \autoref{cor:caterpillar_convex_dtt} we will see that, in both $\dtt_m$ and $\rnni$, any two caterpillar trees are connected by a shortest path that is a caterpillar path.
...
\begin{proof}
	...
	More specifically, we consider the extended ranked versions $T_r$ and $R_r$ of the given trees $T$ and $R$.

	Let the leaf $a_k := \argmax_{a_1, \ldots, a_n}\{\rank(a_i)_{R_r} | \rank(a_i)_{R_r} \neq \rank(a_i)_{T_r}\}$ be the leaf with parent with maximum rank in $R_r$ among those that are not in the same position in $T_r$ and $R_r$.

	Let $a_j$ be a node with $\rank(a_j)_{T_r} = \rank(a_k)_{T_r} + 1$
	We define $T'_r$ to be the caterpillar tree resulting from $T_r$ by performing an $\nni$ move or rank move exchanging the ranks of $(a_k)_{T_r}$ and $(a_j)_{T_r}$.
	An $\nni$ move is necessary, if these two nodes are connected by an edge, otherwise a rank move corresponding to a length move is preformed on $T_r$ to receive $T'_r$.
	By using properties of shortest paths computed by $\findpath$, we now show that $|\fp(R_r,T'_r)| = |\fp(R_r,T_r)| - 1$, proving the theorem.

	Since all clusters of $T_r$ and $T'_r$ induced by nodes of rank less than $\rank(a_k)_{T_r}$ coincide, the paths $\fp(T_r,R_r)$ and $\fp(T'_r,R_r)$ coincide up to a tree $R'_r$, which contains all these clusters.
	\todo{Is this obvious?}
	Moreover, it is $\rank(a_k)_{R'_r}  > \rank(a_j)_{R'_r}$, as this relation does not change on the path computed by $\findpath$ between $R_r$ and $R'_r$.
	Let $S$ be the largest cluster that is shared between $R'_r, T_r,$ and $T'_r$ (see \autoref{fig:proof_caterpillar_convex_rnni}).
	...
	By our assumptions on $T_r$, the clusters considered on $\fp(R_r,T_r)$ in the two iterations following $R'_r$ are first $S \cup \{a_k\}$ and then $S' \cup \{a_j\}$ for some clusters $S$ and $S'$ that are induced by nodes fo the same rank in $T_r$, $T'_r$, and $R'_r$.
	Decreasing the rank of $(S \cup \{a_k\})_{R'_r}$ takes $\rank(S \cup \{a_k\})_{R'_r} - (\rank(S)_{R'_r} + 1)$ $\rnni$ moves, the following iteration for $S' \cup \{a_j\}$ takes $\rank(S' \cup \{a_j\})_{R'_r} + 1 - (\rank(S')_{R'_r} + 2)$.
	Since the parents of $a_k$ and $a_j$, which are exchange while the rank of $(S \cup \{a_k\})_{R'_r}$ decreases, the rank of $(S' \cup \{a_j\}$ decreases by one during that iteration, compared to $R'_r$.
	On $\fp(R_r,T'_r)$ first $\rank(S' \cup \{a_j\})_{R'_r} - (\rank(S')_{R'} + 1)$ $\rnni$ moves decrease the rank of $(S' \cup \{a_j\})_{R'}$, and then $\rank(S \cup \{a_k\})_{R'-r} - (\rank(S)_{R'_r} + 2)$ are needed for $S \cup \{a_k\}$.

	Let $Q_r$ and $Q'_r$ be the trees on $\fp(R_r,T_r)$ and $\fp(R_r,T'_r)$, respectively, after the two iterations following $R'_R$.
	By the above the number of moves on $\fp(R_r,T_r)$ between $R'_r$ and $Q_r$ is one higher than the number of moves on $\fp(R_r,T'_r)$ between $R'_r$ and $Q'_r$.
	Since the trees $Q'_r$ and $T'_r$ can be received from permuting the leaves of $Q_r$ and $T_r$ by the same permutation, namely the transposition $(a_j, a_k)$, the distances between these pairs of trees coincide: $d(Q_R,T_r) = d(Q'_r,T'_r)$.
	We can conclude $d(R_r,T_r) = d(R_r,T'_r) + 1$, and hence $T'_r$ is on a shortest path from $T_r$ to $R_r$.
\end{proof}
