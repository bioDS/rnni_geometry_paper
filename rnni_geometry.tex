...
\begin{abstract}
	A popular assumption in phylogenetic tree inference if the molecular clock hypothesis.
	Using a molecular clock model allows estimating times of evolutionary events.
	A class of models using this assumption are coalescent models, which are used in a variety of applications including cancer and virus evolution.
	Software for reconstructing evolutionary histories under the coalescent model exists, but analyse the results, especially uncertainties of the output, statistical measures are required.
	Therefore, a tree space for coalescent trees and knowledge about its geometry is essential for analysing coalescent trees.

	We present a discrete parameterisation of coalescent trees and establish geometrical properties of the corresponding tree space.
	This tree space is discretisation of the $t$-space of time trees, and our results can hence be transferred to the $t$-space for a fine enough resolution.
	We furthermore discuss the ranked nearest neighbour interchange space as a special case of discrete coalescent trees, and generalise some results that are already known in the space of ranked trees.
	We will find that these two tree spaces have properties promising for statistical analyses of time trees.
\end{abstract}
...
This however can be problematic, as most data contain a lot of noise, which might result in a highly uncertain tree.
In order to statistically analyse reconstructed trees, for example to evaluate the certainty of the timing of the root, knowledge about the geometry of the tree space is essential.
...
Geometrical properties of tree spaces have also been used for analysing behaviour of rogue taxa in a tree \autocite{Cueto2011-bh}, or computing summary trees \autocite{Miller2015-rk}.
...
These tree space can be interpreted a discrete version of the $t$-space.
...
\begin{proof}
	...
	\begin{figure}[ht]
		...
		The clusters $S$ and $S'$ can be found in all three trees.
		...
		At the top $(a_j)_T$ and $(a_k)_T$ are in the two different subtrees $T_r^d$ and $T_r^c$ (the same in $T'_r$ and $R'_r$), which is also true for the disjoint sets $S$ and $S'$.
		Dotted lines represent remaining parts of trees, which are equal in $T_r$ and $T'_r$, but different to $R'_r$.
		Note that the rank difference of $(a_k)_{R'_r}$ and $(a_k)_{R'_r}$ does not need to be one, as it is in $T_r$ and $T'_r$.
	...
	If it otherwise was $\rank(a_k)_{R_r} \leq \rank(a_j)_{R_r}$, it follows $\rank(a_k)_{R_r} \leq \rank(a_j)_{R_r}$, by the definition of $a_k$, and therefore  $\rank(a_k)_{R_r} \leq \rank(a_j)_{R_r} = \rank(a_j)_{T_r} = \rank(a_k)_{T_r} + 1$.
	$\rank(a_k)_{R_r} \leq \rank(a_k)_{T_r} + 1$ however contradicts the definition of $a_k$, hence $\rank(a_j)_{R_r} < \rank(a_k)_{R_r}$.
	With this it also follows $\rank(a_j)_{R'_r} < \rank(a_k)_{R'_r}$.
...
The tree space $\dtt_m$ discussed in this paper is also a promising starting point for establishing a scaling-free tree space, where the distance between trees is the same if all branch lengths are multiplied with the same factor.
Such a tree space is necessary for studies aiming to compare trees on different data sets.
The goal is to compare the branching process through time, without taking leaf labels into account.
