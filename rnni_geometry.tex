...
\begin{proof}
	...
	We now show that by the minimality assumption of $T$ and $R$, $C$ is induced by the node of rank $i$ in $R$.
...
\begin{figure}[ht]
	...
	\caption{Tree in $\dct_4$ on three leaves for which there is not tree with diameter distance $5 = \frac{(n-1)(n-2)}{2} + (m-n+1)(n-1)$ from it}
