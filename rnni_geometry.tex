...
A tree graph that is very similar and has been studied extensively \tocite, is the $\nni$ graph on trees without times where $\nni$ moves are allowed on every edge.
...
\begin{proof}
	...
	We now consider the first step of $\findpath$ applied to compute a shortest path from $T'$ to $R$.
	...
	As $\findpath$ preserves clusters, $C$ is present in every tree on $\fp(T,R)$, and hence $\fp(T',R)$, up to $\hat T$.
	It follows $d(T,R) = d(R,\hat T) + d(\hat T, T)$ and $d(T',R) = d(R,\hat T) + d(\hat T, T')$.
	Now consider $\fp(T', \hat T)$.
	The fist iteration of $\findpath$ on $T'$ and $\hat T$ considers the cluster $C$, as all cluster induced by nodes below $(C)_{T'}$ coincide in $\hat T$ and $T'$.
	To construct this cluster in $T'$, there is just one $\nni$ move needed, which results in the tree $T$.
