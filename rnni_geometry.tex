...
\begin{proof}
	...
	\todo{Do we need the notion $(T)_i$ anywhere else? If so, we need to deifne it in the introduction}
	...
	And since the rank of thw most recent common ancestor decreases when running $\findpath$ until it reaches $i$, the maximum number of moves in each iteration $i$ is $m-i$.
	...
	\todo{briefly describe the example in words}
	\begin{figure}[ht]
