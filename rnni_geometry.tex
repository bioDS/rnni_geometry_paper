...
\begin{abstract}
	Most tree reconstruction methods base on tree search algorithm.
	...
	This is especially the case in coalescence theory, where the order of divergence events plays an important role.

	It has recently been established that the space of ranked nearest neighbour interchange trees has shortest path that can be computed in polynomial time, unlike all previously known tree spaces that base on tree-rearrangement operations.
	In this paper, we go gone step further and do not only consider ranked trees, but trees where nodes are assigned discrete times.
	We show that shortest paths in this space can be computed in polynomial time and establish some geometrical properties of this tree space.
	Since the tree space considered in this paper can be seen as a generalisation of the space of ranked nearest neighbour interchange trees, most of our results apply to both.
\end{abstract}
...
\todo{add Co-authors}
\summary{Why we want coalescent trees}
...
Leaves of these trees represent for example different species, or different genes within a species.
Internal nodes refer to evolutionary events such as speciation or divergence events.
In order to gain knowledge about the evolutionary history in time, evolutionary trees where nodes are equipped with times are of interest in various applications.

\summary{Why discrete coalescent trees -- Coalescence}
\todo{Add more motivation for coalescent model to this paragraph}
A popular model for reconstructing genealogical trees to infer relationships of a sample of genes or chromosomes is the coalescent model \tocite.
This model considers evolution backwards in time, and hence the evolutionary history from the time of the given samples back back to its most recent common ancestor.
Commonly used reconstruction methods using MCMC \tocite and Maximum Likelihood \tocite implement the coalescent model to infer trees where nodes are assigned times, representing the times of the associated evolutionary events.

\summary{How recent results about $\rnni$ make this space interesting and that it is strongly connected to $\dtt_m$}
Most of these reconstruction algorithms rely on tree search algorithm.
An objective function, such as the posterior probability or the likelihood function \tocite, is optimised by searching a tree space.
Under the coalescent model, tree spaces where internal nodes are ordered according to times are used.
Examples of such tree spaces include the BHV-space \tocite, t-space \tocite, and $\tau$-space \tocite, which use different parameterisations for trees.
The problem of these spaces is however, that computing distances is either $\np$-hard, or shortest paths lack biological meaningfulness.

\summary{Why we want to investigate geometrical properties of $\dtt_m$ and $\rnni$}
Understanding the geometry and properties of shortest paths in such tree spaces is essential for tree search algorithms, especially for developing those and evaluating inferred trees.
Methods estimating whether a chain converged in the bayesian tree inference approach, for example, need tree space where distances can be computed efficiently \tocite. %ESS RWTY (Rob Lanfaer)
Summarising a sample of trees also requires a tree space with this property, as well as shortest paths that preserve information share between trees.
Some tree spaces like the BHV space and the $\tau$-space suffer this and often produce star trees as summaries for tree samples \tocite.
It is therefore desirable to find a tree space where distances can be computed efficiently and shortest paths are biologically meaningful.
...
This space can be seen as a generalisation of the ranked nearest neighbour interchange ($\rnni$) space, as introduced in \autocite{Collienne2020-iu}.
We investigate geometrical properties of both spaces, using the result of \autocite{Collienne2020-iu}, specifically the algorithm $\findpath$ for computing shortest paths.
The space $\dtt_m$ does have the desired properties mentioned above, including efficiently computable shortest paths that preserve biological information shared between trees.
After a technical introduction (\autoref{section:technical_introduction}), we discuss how $\findpath$ can be applied to discrete coalescent trees, generalising the results for $\rnni$ in \autoref{section:fp_dtt}.
We then analyse some geometrical properties of both tree spaces $\dtt_m$ and $\rnni$.
At first we discuss the cluster property (\autoref{section:cluster_property}).
In \autoref{section:caterpillar_convex} we then consider a subset of trees in $\dtt_m$ for which we are able to compute distances even more efficiently than with $\findpath$ in $\rnni$.
Following that, we establish the diameter of $\dtt_m$ and briefly discuss its radius.
We finish this paper with a section proposing directions for further research and open problems (\autoref{section:open_problems}).
...
An important tool for establishing those is the algorithm $\findpath$, which has been introduced by \textcite{Collienne2020-iu} for $\rnni$ and shown to also produce shortest paths in $\dtt_m$.
We provide an implementation of this algorithm in $\rnni$ on GitHub \tocite.
In the following we discuss the connection of the $\rnni$ graph and a well-known algebraic structure, the partition lattice.
...
We have seen in \autoref{section:diameter} that there is no algorithm with better worst-case running time for computing shortest paths.
It might however be possible to computed distances more efficiently.
In \autoref{section:caterpillar_convex} we established a way for computing distances between caterpillar trees in $\O(n \log n)$.
...
Another question left open in this paper is the radius of $\dtt_m$.
...
The space $\dtt_m$ generalises $\rnni$ by allowing internal nodes to have integer-valued time differences.
Therefore, we introduced the parameter $m$ to bound the height of a tree in the space of discrete coalescent trees in order to get a finite space.
...
Combining these two parameters $m$ and $\rho$ gives a space of discrete coalescent trees where different moves have different weights.
This tree space is relevant for practical applications, where for example more is known about the tree topology but the uncertainty of the timing of internal nodes high.
Such a tree space is therefore an interesting subject of further studies.
...
Some studies aim to compare two trees on different data sets, like for example \todo{Paper from Tanja Stadler about comparing HIV and Influenza trees?} \tocite.
Such a comparison aims to compare the branching process through time.
Since the given trees arise from different data set, a scaling-free tree space is desired. 
Since the tree space $\rnni$ and $\dtt_m$ have shortest paths computable in polynomial time, they might be a good starting point for further research with the goal of finding a scaling-free tree space where distances can be computed efficiently.
