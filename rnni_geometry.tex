...
\begin{proof}
	...
	\end{align}
	Then it follows by induction that $\hat d$ and $d$ coincide, using the existence of a shortest path consisting only of caterpillar trees (\autoref{thm:caterpillar_convex_rnni}).

	For proving Inequality~\ref{eq:distance_proof}, we first prove $p_{T'} \geq p_T - 1$ and then $m_{T'} \leq m_t + 1$.
	...
	\todo{indices in sets should coincide in definition of $M_T$ and down here in the conditions!}
