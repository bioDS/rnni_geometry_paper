...
The evolutionary history is hence derived from the time of the given samples back to its most recent common ancestor, resulting in trees where every internal node is assigned a unique time.
...
Examples of such tree spaces include the BHV-space \autocite{Billera2001-rj}, t-space, and $\tau$-space \autocite{Gavryushkin2016-uu}, which use different parameterisations for trees.
The problem of these spaces is however, that computing distances is either $\np$-hard (t-space), or shortest paths lack biological meaningfulness.
...
Therefore, summarising tree samples often results in star trees in these spaces, which do not contain any biological information of the sample.
...
Investigating these properties of tree spaces is essential for tree search algorithms, and especially for developing these algorithms and evaluating inferred trees.
...
Summarising a sample of trees, for example a posterior sample, also requires a tree space with this property, as well as shortest paths that preserve information shared between trees, in order to receive a biological meaningful tree.
...
Alternatively, methods such as the maximum clade credibility tree \autocite{Heled2013-sv} or consensus trees \autocite{Margush1981-ba} are used to summarise tree samples.
...
We investigate geometrical properties of both spaces $\dtt_m$ and $\rnni$, using the algorithm $\findpath$ for computing shortest paths as introduced by \textcite{Collienne2020-iu}.
...
After introducing notations used throughout this paper (\autoref{section:technical_introduction}), we discuss how $\findpath$ can be applied to discrete coalescent trees, generalising the results for $\rnni$ in \autoref{section:fp_dtt}.
We then analyse some geometrical properties of both tree spaces $\dtt_m$ and $\rnni$ (\autoref{section:geometry}).
Therefore, we first discuss the cluster property in \autoref{section:cluster_property}.
In \autoref{section:caterpillar_convex} we then consider a subset of trees for which we are able to compute $\rnni$ distances even more efficiently than with $\findpath$.
Following that, we establish the diameter of $\dtt_m$ and $\rnni$ and briefly discuss their radius.
